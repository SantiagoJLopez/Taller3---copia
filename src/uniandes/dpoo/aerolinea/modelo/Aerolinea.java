...
public class Aerolinea
{
    ...
    public Vuelo getVuelo( String codigoRuta, String fechaVuelo )
    {
    	...
    	for (Vuelo vuelo: vuelos) {
    		String cod = vuelo.getRuta().getCodigoRuta();
    		String fecha = vuelo.getFecha();
    		if ((cod == codigoRuta) && (fecha == fechaVuelo)) {
    			return vuelo;
    		}
    	}
        return null;
    ...
    public Collection<Tiquete> getTiquetes( )
    {
    	...
    	for (Vuelo vuelo: vuelos) {
    		HashMap<String, Tiquete> tiquetesVuelo = vuelo.getTiquetes();
    		Collection<String> codigos = tiquetesVuelo.keySet();
    		for (String codigo: codigos) {
    			Tiquete tiquete = tiquetesVuelo.get(codigo);
    ...
    public void programarVuelo( String fecha, String codigoRuta, String nombreAvion ) throws Exception
    {
    	...
    	if (avionElegido == null) {
    		throw new Exception("La aerolínea no posee un avión con el nombre " + nombreAvion);
    	}
    	}
    	Collection<String> codigosRutas = rutas.keySet();
    	for (String codigo: codigosRutas) {
    		if(codigo == codigoRuta) {
    			rutaElegida = rutas.get(codigo);
    		}
    	}
    	if (rutaElegida == null) {
    		throw new Exception("La aerolínea no maneja una ruta con el código "+ codigoRuta);
    	}
    	vuelos.add(new Vuelo(avionElegido, fecha, rutaElegida));
    }
    ...
    public int venderTiquetes( String identificadorCliente, String fecha, String codigoRuta, int cantidad ) throws VueloSobrevendidoException, Exception
    {
    	...
    	if(rutas.containsKey(codigoRuta)) {
    		if (clientes.containsKey(identificadorCliente)) {
    			...
    			if (vuelo == null) {
    				throw new Exception("La aerolínea no posee ese vuelo");
    			}
    			else {
					vuelo.venderTiquetes​(clientes.get(identificadorCliente), calculadora, cantidad);
					return cantidad;
    			}
    		}
    		else {
    			throw new Exception("La aerolínea no tiene registro de un cliente " + identificadorCliente);
    		}
    	}
    	else {
    		throw new Exception("La aerolínea no maneja la ruta con el código " + codigoRuta);
    	}
    }
    ...
    {
        // TODO Implementar el método
        return String.valueOf(clientes.get(identificadorCliente).calcularValorTotalTiquetes());
    }
